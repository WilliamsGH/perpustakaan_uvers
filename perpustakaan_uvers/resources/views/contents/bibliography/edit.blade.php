                    ...
                    <div class="col bg-white mb-3">
                        <div class="row">
                            ...
                            <div class="col-md-6 ps-4 pe-4 ps-md-5">
                                ...
                                <input type="text" class="form-control" name="publisher" value="{{ $book_id->publisher }}" />
                                <label for="">Kala Terbit</label>
                                <input type="text" class="form-control" name="publish_period" value="{{ $book_id->publish_period }}" />
                                <label for="">Tempat Terbit</label>
                                <input type="text" class="form-control" name="publish_place" value="{{ $book_id->publish_place }}" />
                                <label for="">Kategori Buku</label>
                                <select name="category_id" class="form-select" required>
                                    @foreach ($category_ids as $category_id)
                                        <option value="{{$category_id->id}}"  {{ ($book_id->category_id == $category_id->id) ? 'selected' : ''; }}  >{{$category_id->name}}</option>
                                    @endforeach
                                </select>
                                <label for="">Bahasa</label>
                                <select name="language_id" class="form-select" required>
                                    @foreach ($language_ids as $language_id)
                                        <option value="{{$language_id->id}}"  {{ ($book_id->language_id == $language_id->id) ? 'selected' : ''; }}>{{$language_id->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-6 mb-3 ps-4 pe-4 ps-md-0 pe-md-5">
                                ...
                                <input type="text" class="form-control" name="publish_year" value="{{ $book_id->publish_year }}" />
                                <label for="">No Panggil</label>
                                <input type="text" class="form-control" name="internal_reference" value="{{ $book_id->internal_reference }}" />
                                <label for="">Tipe Buku</label>
                                <input type="text" class="form-control" name="type" value="{{ $book_id->type }}" />
                                <label for="">Stock Buku</label>
                                <input type="number" class="form-control" name="stock" value="{{ $book_id->stock }}" required/>
                            </div>
                        </div>
